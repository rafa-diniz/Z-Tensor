...
def test_codec_fidelity(args):
    ...
    print(f"{'Video Source':<20} | {'PSNR score (dB)':<20} | {'SSIM score':<20} | {'Size Original (MB)':<20} | {'Size Z-Tensor Encoded (MB)':<20} | {'Encode Time (s)':<20} | {'Decode Time (s)':<20}")
    print("-" * 170)
    ...
    for video_path in videos:
        ...
        if avg_psnr == "Lossless" and avg_ssim == "Lossless":
            print(f"{os.path.basename(video_path):<30} | {'Lossless':<20} | {'Lossless':<20}  | {size_orig_mb:<20.1f} | {size_ztensor_mb:<20.1f} | {encode_time:20.1f} | {decode_time:20.1f}")
        else:
            print(f"{os.path.basename(video_path):<30} | {avg_psnr:<20.2f} | {avg_ssim:<20.2f} | {size_orig_mb:<20.1f} | {size_ztensor_mb:<20.1f} | {encode_time:20.1f} | {decode_time:20.1f}")
